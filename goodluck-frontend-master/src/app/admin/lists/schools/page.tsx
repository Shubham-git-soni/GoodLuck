...
const defaultForm = (): NewSchoolForm => ({
  name: "", state: "", city: "", board: "", strength: "",
  email: "", contactNo: "", address: "",
  contacts: [emptyContact()],
  station: "", salesTarget: "", prescribeSubjects: [],
});

// ─── Form field components — defined OUTSIDE main component so they never remount on state change ──

function BasicInfoFields({
  form, setForm, modalMode, isMobile = false,
}: {
  form: NewSchoolForm;
  setForm: (f: NewSchoolForm) => void;
    ...
    <div className="grid gap-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="grid gap-2">
          ...
          {isMobile ? (
            <NativeSelect disabled={isView} value={form.state} onValueChange={(v) => setForm({ ...form, state: v, city: "" })} placeholder="Select state">
        ...
        <div className="grid gap-2">
          ...
          {isMobile ? (
            <NativeSelect disabled={isView || !form.state} value={form.city} onValueChange={(v) => setForm({ ...form, city: v })} placeholder={form.state ? "Select city" : "Select state first"}>
              {getCitiesForState(form.state).map((c) => <NativeSelectOption key={c} value={c}>{c}</NativeSelectOption>)}
            </NativeSelect>
          ) : (
            <Select disabled={isView || !form.state} value={form.city} onValueChange={(v) => setForm({ ...form, city: v })}>
              ...
              <SelectContent>{getCitiesForState(form.state).map((c) => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent>
            </Select>
          )}
        </div>
        <div className="grid gap-2">
          ...
          {isMobile ? (
            <NativeSelect disabled={isView} value={form.board} onValueChange={(v) => setForm({ ...form, board: v })} placeholder="Select board">
              {dropdownOptions.boards.map((b) => <NativeSelectOption key={b} value={b}>{b}</NativeSelectOption>)}
            </NativeSelect>
          ) : (
            <Select disabled={isView} value={form.board} onValueChange={(v) => setForm({ ...form, board: v })}>
              <SelectTrigger className={isView ? "bg-muted/50" : ""}><SelectValue placeholder="Select board" /></SelectTrigger>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="grid gap-2">
          ...
          <Input type="number" value={form.strength} onChange={(e) => setForm({ ...form, strength: e.target.value })}
            placeholder="e.g. 800" readOnly={isView} autoComplete="off" className={isView ? "bg-muted/50 pointer-events-none" : ""} />
        </div>
        <div className="grid gap-2">
          <Label>School Contact No.</Label>
          <Input type="tel" inputMode="numeric" maxLength={10} value={form.contactNo}
            onChange={(e) => setForm({ ...form, contactNo: e.target.value.replace(/\D/g, "") })}
            placeholder="10-digit number" readOnly={isView} autoComplete="off" className={isView ? "bg-muted/50 pointer-events-none" : ""} />
        </div>
      </div>
      ...
      <div className="grid gap-2">
        ...
        <Input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })}
          placeholder="school@example.com" readOnly={isView} autoComplete="off" className={isView ? "bg-muted/50 pointer-events-none" : ""} />
      </div>

      <div className="grid gap-2">
        <Label>Address</Label>
        <Textarea value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })}
          placeholder="Full school address" rows={2} className={isView ? "bg-muted/50 pointer-events-none" : ""} readOnly={isView} />
      </div>
    </div>
  );
}

function ContactPersonsFields({
  form, setForm, modalMode, isMobile = false,
}: {
  form: NewSchoolForm;
  setForm: (f: NewSchoolForm) => void;
  ...
  return (
    <div className="grid gap-4">
      {form.contacts.map((contact, idx) => (
        <div key={contact.id} className="border rounded-xl p-4 relative bg-muted/20">
          <div className="flex items-center justify-between mb-3">
            ...
            {form.contacts.length > 1 && !isView && (
              <Button type="button" variant="ghost" size="icon" className="h-7 w-7 text-destructive hover:bg-destructive/10" onClick={() => removeContact(idx)}>
          ...
          <div className="grid gap-3">
            <div className="grid gap-2">
              ...
              <Input value={contact.name} onChange={(e) => updateContact(idx, "name", e.target.value)}
                placeholder="Contact person name" readOnly={isView} autoComplete="off"
                className={isView ? "bg-muted/50 pointer-events-none" : ""} />
            </div>
            <div className="grid gap-2">
              <Label>Designation</Label>
              {isMobile ? (
                <NativeSelect disabled={isView} value={contact.designation} onValueChange={(v) => updateContact(idx, "designation", v)} placeholder="Select designation">
                  {dropdownOptions.contactRoles.map((r) => <NativeSelectOption key={r} value={r}>{r}</NativeSelectOption>)}
                </NativeSelect>
              ) : (
                <Select disabled={isView} value={contact.designation} onValueChange={(v) => updateContact(idx, "designation", v)}>
                  <SelectTrigger className={isView ? "bg-muted/50" : ""}><SelectValue placeholder="Select designation" /></SelectTrigger>
                  <SelectContent>{dropdownOptions.contactRoles.map((r) => <SelectItem key={r} value={r}>{r}</SelectItem>)}</SelectContent>
                </Select>
              )}
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="grid gap-2">
                ...
                <Input type="tel" inputMode="numeric" maxLength={10} value={contact.mobile}
                  onChange={(e) => updateContact(idx, "mobile", e.target.value.replace(/\D/g, ""))}
                  placeholder="10-digit number" readOnly={isView} autoComplete="off"
                  className={isView ? "bg-muted/50 pointer-events-none" : ""} />
              </div>
              <div className="grid gap-2">
                <Label>Email</Label>
                <Input type="email" value={contact.email} onChange={(e) => updateContact(idx, "email", e.target.value)}
                  placeholder="Email address" readOnly={isView} autoComplete="off"
                  className={isView ? "bg-muted/50 pointer-events-none" : ""} />
              </div>
            </div>
          </div>
      ...
      {!isView && (
        <Button type="button" variant="outline" className="w-full border-dashed text-muted-foreground hover:text-foreground" onClick={addContact}>
          <UserPlus className="h-4 w-4 mr-2" />Add Another Contact Person
        </Button>
      )}
    </div>
  );
...
function SalesInfoFields({
  form, setForm, modalMode, stations, isMobile = false,
}: {
  form: NewSchoolForm;
  setForm: (f: NewSchoolForm) => void;
  modalMode: "add" | "edit" | "view";
  ...
  return (
    <div className="grid gap-4">
      <div className="grid gap-2">
        ...
        {isMobile ? (
          <NativeSelect disabled={isView} value={form.station} onValueChange={(v) => setForm({ ...form, station: v })} placeholder="Select station">
            {stations.map((st) => <NativeSelectOption key={st} value={st}>{st}</NativeSelectOption>)}
          </NativeSelect>
        ) : (
          <Select disabled={isView} value={form.station} onValueChange={(v) => setForm({ ...form, station: v })}>
            <SelectTrigger className={isView ? "bg-muted/50" : ""}><SelectValue placeholder="Select station" /></SelectTrigger>
            <SelectContent>{stations.map((st) => <SelectItem key={st} value={st}>{st}</SelectItem>)}</SelectContent>
          </Select>
        )}
      </div>
      ...
      <div className="grid gap-2">
        ...
        <div className="relative">
          ...
          <Input type="number" value={form.salesTarget} onChange={(e) => setForm({ ...form, salesTarget: e.target.value })}
            placeholder="e.g. 500000" readOnly={isView} autoComplete="off"
      ...
      <div className="grid gap-2">
        ...
        {isView ? (
          <div className="flex flex-wrap gap-1.5 p-3 rounded-lg bg-muted/50 min-h-10">
            {form.prescribeSubjects.length ? form.prescribeSubjects.map(s => (
          ...
          <MultiSelect
            ...
            value={form.prescribeSubjects}
            onChange={(v) => setForm({ ...form, prescribeSubjects: v })}
...
function TabbedForm({
  form, setForm, modalMode, stations, activeTab, setActiveTab, isMobile = false,
}: {
  form: NewSchoolForm;
  setForm: (f: NewSchoolForm) => void;
  modalMode: "add" | "edit" | "view";
      ...
      <TabsList className="grid w-full grid-cols-3 mb-4">
        ...
        <TabsTrigger value="contacts">
          ...
          {form.contacts.length > 0 && (
            <Badge variant="secondary" className="ml-1.5 h-5 px-1.5 text-[10px]">{form.contacts.length}</Badge>
          )}
        </TabsTrigger>
      </TabsList>
      <TabsContent value="basic">
        <BasicInfoFields form={form} setForm={setForm} modalMode={modalMode} isMobile={isMobile} />
      </TabsContent>
      <TabsContent value="sales">
        <SalesInfoFields form={form} setForm={setForm} modalMode={modalMode} stations={stations} isMobile={isMobile} />
      </TabsContent>
      <TabsContent value="contacts">
        <ContactPersonsFields form={form} setForm={setForm} modalMode={modalMode} isMobile={isMobile} />
      </TabsContent>
    </Tabs>
  );
}
...
export default function SchoolListPage() {
  ...
  const [form, setForm] = useState<NewSchoolForm>(defaultForm());
  ...
  useEffect(() => {
    ...
    const handleTouch = (e: TouchEvent) => {
      if (sheetPanelRef.current && sheetPanelRef.current.contains(e.target as Node)) return;
      e.stopPropagation();
  ...
  const stations = Array.from(new Set(schools.map(s => s.station))).filter(Boolean) as string[];
  ...
  const handleSubmitSchool = () => {
    ...
    if (!form.name || !form.city || !form.board) {
    ...
    const salesTargetNum = parseFloat(form.salesTarget) || 0;
    const prescribedBooks = form.prescribeSubjects.map(sub => ({
    ...
    if (modalMode === "add") {
      const newEntry: School = {
        ...
        name: form.name, city: form.city, board: form.board,
        strength: parseInt(form.strength) || 0, address: form.address,
        state: form.state, station: form.station, assignedTo: "",
        visitCount: 0, businessHistory: [], discountHistory: [],
        salesPlan: { targetRevenue: salesTargetNum, subjects: form.prescribeSubjects, expectedConversion: 0 },
        prescribedBooks,
        contacts: form.contacts.map((c, i) => ({
          id: c.id, name: c.name, role: c.designation, email: c.email, phone: c.mobile, isPrimary: i === 0,
        })),
      } as any;
      ...
      setSchools(prev => prev.map(s => s.id === activeSchoolId ? {
        ...s, name: form.name, city: form.city, board: form.board,
        strength: parseInt(form.strength) || 0, address: form.address, station: form.station,
        salesPlan: { ...s.salesPlan, targetRevenue: salesTargetNum, subjects: form.prescribeSubjects },
        prescribedBooks,
        contacts: form.contacts.map((c, i) => ({
          id: c.id, name: c.name, role: c.designation, email: c.email, phone: c.mobile, isPrimary: i === 0,
        })),
      } : s));
      toast.success(`School "${form.name}" updated successfully!`);
    }
    setIsMobileSheetOpen(false);
  ...
  const SCHOOL_COLUMNS: GridColumn<School>[] = [
    ...
    { key: "station", header: "Station", width: 130, sortable: true, filterable: true, render: (val) => <span className="text-xs">{val || "—"}</span> },
    {
  ...
  const rowActions: RowAction<School>[] = [
    {
      ...
      onClick: (s) => { if (window.innerWidth < 768) setViewSchool(s); else openModal("view", s); }
    },
  ...
  return (
    <PageContainer>
        ...
        <div className="fixed inset-0 z-[100] flex flex-col justify-end">
          ...
          <div className="relative bg-background rounded-t-3xl shadow-2xl animate-in slide-in-from-bottom duration-300" style={{ maxHeight: "92dvh", display: "flex", flexDirection: "column" }}>
            ...
            <div className="flex items-center justify-between px-5 pt-1 pb-3 shrink-0">
              <h2 className="text-lg font-bold tracking-tight flex items-center gap-2"><SchoolIcon className="h-4 w-4" /> School Details</h2>
              <button onClick={() => setViewSchool(null)} className="h-8 w-8 rounded-full bg-muted flex items-center justify-center"><X className="h-4 w-4" /></button>
            </div>
                    ...
                    {viewSchool.contacts.map((contact, idx: number) => (
                      <div key={idx} className="p-3 rounded-xl bg-muted/30 border border-border/50">
                        ...
                        <div className="flex gap-4 mt-2">
                          ...
                          {contact.email && <div className="flex items-center gap-1.5 text-xs text-muted-foreground min-w-0"><Mail className="h-3 w-3 shrink-0" /><span className="truncate">{contact.email}</span></div>}
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            <div className="shrink-0 px-5 pb-6 pt-3 border-t flex gap-3">
              <Button variant="outline" className="flex-1 h-12 rounded-2xl" onClick={() => { setViewSchool(null); router.push(`/admin/lists/schools/${viewSchool.id}`); }}>Full Profile</Button>
              <Button className="flex-1 h-12 rounded-2xl" variant="outline" onClick={() => setViewSchool(null)}>Close</Button>
            </div>
          </div>
        </div>
      )}
      ...
      {isMobileSheetOpen && (
        <div className="fixed inset-0 z-[100] flex flex-col justify-end">
          <div ref={sheetBackdropRef} className="absolute inset-0 bg-black/60" />
          <div
            ref={sheetPanelRef}
          ...
          >
            ...
            <div style={{ flex: 1, overflowY: "auto", WebkitOverflowScrolling: "touch" as any }} className="px-5 pb-4">
              <TabbedForm form={form} setForm={setForm} modalMode={modalMode} stations={stations} activeTab={activeTab} setActiveTab={setActiveTab} isMobile />
            </div>
      ...
      {/* ── Desktop Dialog ── */}
      <Dialog open={isDesktopDialogOpen} onOpenChange={(o) => { setIsDesktopDialogOpen(o); if (!o) { setForm(defaultForm()); setActiveTab("basic"); } }}>
        <DialogContent className="!w-[75vw] !max-w-[75vw] h-[80vh] max-h-[80vh] flex flex-col p-0 gap-0 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto px-6 py-5 bg-muted/10">
            <TabbedForm form={form} setForm={setForm} modalMode={modalMode} stations={stations} activeTab={activeTab} setActiveTab={setActiveTab} />
          </div>
          <DialogFooter className="px-6 py-4 border-t bg-background shrink-0">
            <Button variant="outline" onClick={() => { setIsDesktopDialogOpen(false); setForm(defaultForm()); setActiveTab("basic"); }}>Close</Button>
            {modalMode !== "view" && (
                ...
                {row.contacts.map((contact, idx) => (
                  <div key={idx} className="flex flex-col gap-1 p-3 rounded-lg bg-background border border-border/50 shadow-sm min-w-[200px]">
                    ...
                    <div className="flex items-center gap-2 mt-2 text-xs">
                      <Phone className="h-3 w-3 text-muted-foreground" /><span>{contact.phone || "N/A"}</span>
                    </div>
                    {contact.email && (
                      <div className="flex items-center gap-2 text-xs">
                        <Mail className="h-3 w-3 text-muted-foreground" /><span className="truncate max-w-[150px]" title={contact.email}>{contact.email}</span>
                      </div>
                    )}
                  </div>
                ))}
